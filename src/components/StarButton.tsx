...
function StarButton({ snippetId }: { snippetId: Id<"snippets"> }) {
  ...
  const isStarred = useQuery(api.snippets.isSnippetsStarered, { snippetId });
  const starCount = useQuery(api.snippets.getSnippetsStarCount, { snippetId });
  const star = useMutation(api.snippets.starSnipet);
